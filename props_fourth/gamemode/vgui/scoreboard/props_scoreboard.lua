...
function PANEL:Init()
	...
	self:SetSize( ScrW() / (1440 / 1300), ScrH() / (900 / 820) )
	--self:SetPos( 70, 30 )
	self:SetPos( ScrW() / (1440 / 70), ScrH() / (900 / 30) )
	self:MakePopup()
	...
	self.MainHeader:SetWide( ScrW() / (1440 / 1300) )
	--self.MainHeader:SetTall( 60 )
	self.MainHeader:SetTall( ScrH() / (900 / 60) )
	...
	self.ContentGap:SetWide( ScrW() / (1440 / 1300) )
	--self.ContentGap:SetTall( 20 )
	self.ContentGap:SetTall( ScrH() / (900 / 20) )
	...
	self.ContentGap.ArrowLeft:SetWide( ScrW() / (1440 / 24) )
	...
	self.ContentGap.ArrowRight:SetWide( ScrW() / (1440 / 24) )
	...
	self.Content:SetWide( ScrW() / (1440 / 1300) )
	--self.Content:SetTall( 740 )
	self.Content:SetTall( ScrH() / (900 / 740) )
...
function PANEL:Update()
	...
	for i=1,#TeamsScoreboard[ LocalPlayer().ScoreboardArrow ] do
		...
		self.Content[ "Panel_" .. i ]:SetTall( ScrH() / (900 / 740) )
		...
		if teamCount != i then
			...
			self.Content[ "Padding_" .. i ]:SetTall( ScrH() / (900 / 740) )
		...
		self.Content[ "Panel_" .. i ].TeamHeader:SetTall( ScrH() / (900 / 30) )
		...
		for a,b in pairs( team.GetPlayers( v ) ) do
			...
			self.Content[ "Panel_" .. i ].TeamContent.PlayerRow:SetTall( ScrH() / (900 / 37) )
