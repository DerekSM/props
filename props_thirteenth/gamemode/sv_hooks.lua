...
function GM:StartBattle( pl, target, kills, props )
	...
	PROPKILL.ServerBattleTime = PROPKILL.BattleTime * 60
	timer.Create( "props_Battlecountdown", 1, PROPKILL.BattleTime * 60, function()
...
function GM:PauseBattle( pl )
	...
	if fighter then
		...
		timer.Create( "props_BattleResume", 1, PROPKILL.Config[ "battle_pausetime" ].default, function()
			if not PROPKILL.Battling then return end
