...
function PANEL:Init()
	...
	for k,v in ipairs( PROPKILL.RecentBattles or {} ) do
		v.Names = v.Inviter .. " : " .. v.Invitee 
		
		
			-- todo:
		--surface.SetFont( "props_HUDTextTiny" )
		--local namessizew, namessizeh = surface.GetTextSize( v.Names )
		print( v.Names, namessizew, #v.Names )
		-- if greater than 195
		-- differnet on different resoltuons.. hmmm
		
		local setnames = false
		
		--[[if namessizew >= 195 then
			setnames = true
		end]]
		...
		self.RightPanel.RecentContentPanel[ "recent" .. k ].ContentNames:SetFont( not setnames and "props_HUDTextTiny" or "props_HUDTextVeryTiny" )
		...
		self.RightPanel.RecentContentPanel[ "recent" .. k ].ContentWinner:SetFont( not setnames and "props_HUDTextTiny" or "props_HUDTextVeryTiny" )
	...
	self.RightPanel.StartButton.DoClick = function( pnl )
		...
		local pl = self.LeftPanel.ListView:GetLine( self.LeftPanel.ListView:GetSelectedLine() ):GetValue( 2 )
		RunConsoleCommand( "props_requestbattle", pl, self.LeftPanel.FragLimitPanel.Wang:GetValue(), self.LeftPanel.TimePanel.Wang:GetValue() )
	end
