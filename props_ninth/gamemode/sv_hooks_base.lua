...
function GM:PlayerInitialSpawn( pl )
	...
	timer.Create( "props_NetworkUpdatedConfig" .. pl:UserID(), 5, 1, function()
		if not IsValid( pl ) then return end
...
function GM:PlayerDisconnected( pl )
	...
	if PROPKILL.Battling and PROPKILL.Battlers[ "inviter" ] == pl then
		GAMEMODE:EndBattle( pl, PROPKILL.Battlers[ "invitee" ], pl, nil, PROPKILL.Battlers[ "invitee" ] )
	elseif PROPKILL.Battling and PROPKILL.Battlers[ "invitee" ] == pl then
		GAMEMODE:EndBattle( PROPKILL.Battlers[ "inviter" ], pl, pl, nil, PROPKILL.Battlers[ "inviter" ] )
	end
