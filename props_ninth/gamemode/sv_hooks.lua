...
function GM:EndBattle( pl, pl2, forfeiter, stopped, winner, score, msg )
	...
	if forfeiter then--and IsValid( forfeiter ) then
		...
		PROPKILL.RecentBattles[ #PROPKILL.RecentBattles + 1 ] = { time = os.time(), Inviter = pl:Nick(), Invitee = pl2:Nick(), forfeit = true, forfeiter = forfeiter, winner = winner, score = "FF" }
	elseif stopped then
		for k,v in pairs( player.GetAll() ) do
		...
		PROPKILL.RecentBattles[ #PROPKILL.RecentBattles + 1 ] = { time = os.time(), Inviter = pl:Nick(), Invitee = pl2:Nick(), stopped = true, winner = "Nobody", score = "N/A" }
	elseif winner then
		for k,v in pairs( player.GetAll() ) do
			v:Notify( 0, 6, msg or "The fight has ended" )
		end
		--PROPKILL.RecentBattles[ os.time() ] = { Inviter = pl, Invitee = pl2, winner = winner, score = score }
		PROPKILL.RecentBattles[ #PROPKILL.RecentBattles + 1 ] = { time = os.time(), Inviter = pl:Nick(), Invitee = pl2:Nick(), winner = winner, score = score }
	else
		for k,v in pairs( player.GetAll() ) do
			v:Notify( 0, 6, msg or "The fight has ended" )
		end
