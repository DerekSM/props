...
function GM:DoPlayerDeath( pl, killer, dmginfo )
	...
	if not prop_owner then 
		pl:SetKillstreak( 0 )
		return
	end
	
	print( dmginfo:GetInflictor(), dmginfo:GetAttacker() )
	
	if prop_owner != pl then
		prop_owner:AddFrags( 1 )
		prop_owner:AddKillstreak( 1 )
		--prop_owner:SetKillstreak( prop_owner:GetKillstreak() + 1 )
			-- i can just do total kills instead...
			-- maybe i should keep this because people override player total kills...
		---prop_owner.PlayerHits = ( prop_owner.PlayerHits or 0 ) + 1
		
		---prop_owner:SetAccuracy( math.Clamp( 100 * (prop_owner.PlayerHits / prop_owner.PropSpawns), 0, 100 ) )
	end
	...
	if prop_owner != pl then
		if prop_owner:GetPos():Distance( pl:GetPos() ) >= 4000 then
			...
			--prop_owner:SetLongshots( prop_owner:GetLongshots() + 1 )
		elseif prop_owner:IsFlying() then
			...
			--prop_owner:SetFlybys( prop_owner:GetFlybys() + 1 )
		end
	end
	...
	if PROPKILL.Battling then
		if PROPKILL.Battlers[ "inviter" ]:Kills() >= PROPKILL.BattleAmount then
			self:EndBattle( PROPKILL.Battlers[ "inviter" ], PROPKILL.Battlers[ "invitee" ], nil, PROPKILL.Battlers[ "inviter" ]:Nick() .. " has won the fight! ( " .. PROPKILL.Battlers[ "inviter" ]:GetKillstreak() .. " - " .. PROPKILL.Battlers[ "invitee" ]:GetKillstreak() )
		else
			self:EndBattle( PROPKILL.Battlers[ "invitee" ], PROPKILL.Battlers[ "inviter" ], nil, PROPKILL.Battlers[ "invitee" ]:Nick() .. " has won the fight! ( " .. PROPKILL.Battlers[ "invitee" ]:GetKillstreak() .. " - " .. PROPKILL.Battlers[ "inviter" ]:GetKillstreak() )
		end
	end
