...
function GM:OnSettingChanged( pl, setting_id, s_from, s_to )
	...
	for k,v in pairs( player.GetAll() ) do
		...
		canChange = hook.Call( "PlayerCanChangeSetting", nil, v, setting_id )
		if canChange then
...
function GM:StartBattle( pl, target, kills )
	for k,v in pairs( player.GetAll() ) do
		v:Notify( 0, 6, pl:Nick() .. " has started a battle with " .. target:Nick() .. " to " .. kills .. " kills" )
	end
	...
	timer.Create( "props_Autostopfight", PROPKILL.Config[ "battle_time" ].default, 1, function()
		if not PROPKILL.Battling then return end
...
function GM:EndBattle( pl, pl2, forfeiter, msg )
